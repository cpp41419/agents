'use client';

import { useState } from 'react';
import { useForm } from 'react-hook-form';
import { z } from 'zod';
import { zodResolver } from '@hookform/resolvers/zod';
import { BookText, Loader2, Sparkles } from 'lucide-react';

import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';
import { Input } from '@/components/ui/input';
import { explainTermAction } from '@/app/actions';
import type { ExplainRealEstateTermOutput } from '@/ai/flows';

const formSchema = z.object({
  term: z.string().min(2, {
    message: 'Term must be at least 2 characters.',
  }),
});

export default function TerminologyExplainer() {
  const [explanation, setExplanation] = useState<ExplainRealEstateTermOutput | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      term: '',
    },
  });

  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    setError(null);
    setExplanation(null);

    const result = await explainTermAction({ term: values.term });

    if (result.success && result.data) {
      setExplanation(result.data);
    } else {
      setError(result.message || 'An unknown error occurred.');
    }
    setIsLoading(false);
  }

  return (
    <section className="w-full py-12 md:py-24 lg:py-32 bg-secondary">
      <div className="container px-4 md:px-6">
        <div className="mx-auto max-w-3xl text-center space-y-4">
            <div className="inline-block rounded-lg bg-muted px-3 py-1 text-sm">AI-Powered Explainer</div>
            <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">Demystify Real Estate Jargon</h2>
            <p className="text-muted-foreground md:text-xl/relaxed">
                Confused by terms like "escrow" or "contingency"? Enter any real estate term below, and our AI will provide a simple, easy-to-understand explanation.
            </p>
        </div>

        <Card className="max-w-2xl mx-auto mt-10">
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><BookText className="text-primary"/> Terminology Explainer</CardTitle>
                <CardDescription>Get clear definitions for complex terms.</CardDescription>
            </CardHeader>
            <CardContent>
                 <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        <FormField
                        control={form.control}
                        name="term"
                        render={({ field }) => (
                            <FormItem>
                            <FormLabel>Real Estate Term</FormLabel>
                            <FormControl>
                                <Input placeholder="e.g., Amortization" {...field} />
                            </FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                        />
                        <Button type="submit" disabled={isLoading}>
                            {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}
                            Explain Term
                        </Button>
                    </form>
                </Form>

                {explanation && (
                    <div className="mt-6 p-4 border rounded-lg bg-background">
                        <h3 className="font-bold text-lg text-primary">{form.getValues('term')}</h3>
                        <p className="mt-2 text-foreground/90">{explanation.explanation}</p>
                    </div>
                )}
                {error && (
                    <div className="mt-6 p-4 border rounded-lg bg-destructive/10 text-destructive">
                        <p>{error}</p>
                    </div>
                )}
            </CardContent>
        </Card>
      </div>
    </section>
  );
}
